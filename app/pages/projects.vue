...
import { getProjects } from '~/composables/useContent'
...
const {
  ...
  error,
  refresh
} = await useAsyncData('projects', () => getProjects(), {
  server: true,
  default: () => []
})
</script>
  ...
  <UContainer class="pt-20 pb-24">
    ...
    >
      <div class="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        ...
        <UButton variant="outline" size="lg" to="/articles"> Read the blog </UButton>
      </div>

      <div class="mt-10">
        <LoadingState v-if="pending" message="Loading projects…" />
        <ErrorState
          v-else-if="error"
        ...
        />
        <EmptyState
          v-else-if="Array.isArray(projects) && projects.length === 0"
          title="No projects yet"
          description="I haven’t published any projects yet. Check back soon."
        />

        <div v-else class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          <motion.div
            v-for="project in projects"
            :key="project.slug ?? project.name ?? project.url"
      ...
      <div class="mt-12 flex justify-center">
        <UButton variant="ghost" size="lg" to="/articles"> Continue to the blog </UButton>
      </div>
    </motion.section>
  </UContainer>
</template>
